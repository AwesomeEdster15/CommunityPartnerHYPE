...
if($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if(isset(trim($_POST["stockCount"]))) {
  ...
  if(empty($stockCountError) && empty($reusableError) && empty($imageLinkError) && empty($productNameError) && empty($requestPeriodError) && empty($productLinkError)) {
    ...
    $insertProductTypeSQL = "UPDATE ProductType SET productLink=?, stockCount=?, reusable=?, imageLink=?, productName=?, requestPeriod=? WHERE productName=?";
    if($stmt = mysqli_prepare($dbCon, $insertProductTypeSQL)) {
      #Bind variables to statement as parameters
      mysqli_stmt_bind_param($stmt, "sisssis", $paramproductLink, $paramStockCount, $paramReusable, $paramImageLink, $paramProductName, $paramRequestPeriod, $paramSavedProductName);
      #Set parameters
...
<body>
  ...
  <div class="main">
    ...
    <div class="section1">
      <div>
        <h2>Edit <?php echo $productName; ?></h2>
        <p>Complete this form to edit a product type</p>
